...
const Footer = ({ title }) => {
  ...
  return (
    ...
    >
      ...
      <i className='fas fa-copyright' /> {`${copyrightDate}`} <span><i className='mx-1 animate-pulse fas fa-brush'/> <a href={BLOG.LINK} className='underline font-bold  dark:text-yellow-500 '>{BLOG.AUTHOR}</a>.<br/>

      {BLOG.BEI_AN && <><i className='fas fa-shield-alt' /> <a href='' className='mr-2'>{BLOG.BEI_AN}</a><br/></>}
